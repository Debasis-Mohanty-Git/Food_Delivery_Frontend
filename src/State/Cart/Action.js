...
export const addItemToCart = (reqData) => async (dispatch) => {
    ...
    try {
        const {data}=await axios.put(`/api/cart/add`,reqData.cartItem,{
            headers:{
               Authorization:`Bearer ${reqData.jwt}`
            },
        });
...
export const removeCartItem = ({jwt,cartItemId}) => async (dispatch) => {
    dispatch({ type: REMOVE_CART_ITEM_REQUEST });
    try {
        const {data}=await axios.delete(`/api/cart-item/${cartItemId}/delete`,reqData.data,{
            headers:{
               Authorization:`Bearer ${jwt}`
            },
        });
